<template>
  <div id="wrap">
    ...
    <el-button @click="initView">生成</el-button>
  </div>
</template>
<script>
import go from "gojs";
...
export default {
  ...
  mounted() {
    ...
    this.getFlowData = function() {
      _designer.model.modelData.position = go.Point.stringify(
        _designer.position
      );
    ...
    function updateNodeData(node) {
      var steptext = $("#StepName").val();
      var stepid = $("#StepID").val();
      var stepcolor = $("#StepColor").val();
      _designer.startTransaction("vacate");
      if (steptext !== "") {
        _designer.model.setDataProperty(node.data, "text", steptext);
      }
      if (stepid !== "") {
        _designer.model.setDataProperty(node.data, "key", stepid);
      }
      $("#StepName").val("");
      $("#StepID").val("");
